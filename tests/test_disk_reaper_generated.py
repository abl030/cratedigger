...
transfer record at the end of the cycle. Five invariants, each shipped as
a deterministic pin (``TestDiskReaperDeterministicPins``) AND a generated
property (``TestGeneratedDiskReaperInvariants``):
...
def assert_disk_reaper_invariants(result: DiskReaperRunResult) -> None:
    ...
    if result.empty_stale_path is not None and result.empty_stale_survived:
        raise AssertionError(
            f"stale empty directory should have been pruned: "
            f"{result.empty_stale_path}")
...
class TestGeneratedDiskReaperInvariants(unittest.TestCase):
    """Properties 1-5: within-root, ownership, quarantine, age,
    empty-dir-only pruning -- patrolled together over generated worlds."""
