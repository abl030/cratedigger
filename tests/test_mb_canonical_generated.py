...
This module authorizes a duplicate REMOVAL downstream, so its failure
posture matters more than its success case. The mirror has served wrong
bodies for adversarially-selected MBIDs from a TTL-less cache, and a bogus
UUID answers ``400`` rather than ``404`` at all.
...
R3  A returned survivor is never the requested id. A self-redirect is not a
    merge, and persisting one would collapse the union resolver's
    two-identity probe to one.
...
def check_never_the_requested_id(answer: str | None, requested: str) -> None:
    ...
    if answer is not None and answer == normalize_release_id(requested):
        raise AssertionError(
            ...
            "successor; the DB CHECK would reject persisting that"
        )
