...
reading a spectral field — so scanning it can only make the synchronous
operator surface wait. Measured on doc2 before the skip: 29s for a 12-track
MP3 album, 22s of it the candidate scan.
...
world space around them: layout, codec eligibility, track count, and whether
the audio-integrity gate fired first — the branch that must keep scanning,
because issue #1030 pinned a corrupt candidate's attempt audit as evidence.
...
``tests/test_import_preview_generated.py``.
"""
...
def spectral_scan_boundary_violations(
    *,
    ...
    if not audio_corrupt and layout == "flat" and eligible and not scanned:
        violations.append(
...
class TestSpectralScanBoundaryGenerated(unittest.TestCase):
    ...
    audio. The only stand-ins are the two external edges the module already
    treats as such: the sox/ffmpeg spectral analyzer (through the sanctioned
    ``spectral_detail_analyzer`` kwarg-DI seam) and, for corrupt worlds, the
    ...
    @example(codec_index=0, nested=True, audio_corrupt=False, track_count=2)
    @example(codec_index=1, nested=True, audio_corrupt=False, track_count=1)
    @example(codec_index=0, nested=True, audio_corrupt=True, track_count=1)
    @example(codec_index=0, nested=False, audio_corrupt=False, track_count=1)
    @example(codec_index=2, nested=False, audio_corrupt=False, track_count=1)
    def test_only_a_flat_eligible_candidate_pays_for_a_spectral_scan(
        ...
        layout: Literal["flat", "nested"] = "nested" if nested else "flat"
        scanned: list[str] = []
        ...
        try:
            ...
            for track in range(track_count):
                shutil.copy(
                    ...
                    os.path.join(album, f"{track + 1:02d}.{extension}"),
                )
        ...
        *,
        ...
        return PreimportMeasurement(
            ...
            spectral_audit=SpectralDetail(
                candidate=SpectralAnalysisDetail(attempted=attempted),
