...
class TestFindDownloadDoesNotBlockSearchRefill(unittest.TestCase):
    ...
    def test_parallel_find_download_results_merge_and_log_on_owner_thread(self):
        ...
        self.assertEqual(ctx.counters.find_download_queued, 2)
        self.assertEqual(ctx.counters.find_download_completed, 2)
        self.assertEqual(ctx.counters.browse_time_s, 2.0)
        self.assertEqual(ctx.counters.match_time_s, 1.0)
        self.assertEqual(ctx.counters.peers_browsed, 7)
        self.assertEqual(ctx.counters.peers_browsed_lazy, 1)
        self.assertEqual(ctx.counters.fanout_waves, 3)
        self.assertEqual(ctx.counters.cache_pos_hits, 7)
        self.assertEqual(ctx.counters.cache_neg_hits, 4)
        self.assertEqual(ctx.counters.cache_misses, 12)
