...
class TestAtomicAndExecuteHardening(unittest.TestCase):
    ...
    def test_advisory_scope_never_replays_after_its_backend_dies(self):
        """A session lock is not an authority token a reconnect can retain."""
        ...
        with self.assertRaises(AdvisoryLockSessionLost), db.advisory_lock(
            ...
            pid = self._backend_pid(db)
            _terminate_backend(db.dsn, pid)
            with self.assertRaises(AdvisoryLockSessionLost):
                db._execute("SELECT 1 AS must_not_replay")
            self.assertIs(db.conn, original_conn)

        # The fail-closed scope ends before ordinary one-statement healing
        # resumes; no SQL from inside the locked protocol was replayed.
        cur = db._execute("SELECT 1 AS reconnected_after_scope")
        self.assertEqual(cur.fetchone()["reconnected_after_scope"], 1)
        self.assertIsNot(db.conn, original_conn)

    def test_supersede_transaction_session_loss_is_typed_under_association_locks(self):
        """Replace's real transactional writer cannot retain a dead lock."""
        from lib.pipeline_db import (
            ADVISORY_LOCK_NAMESPACE_IMPORT,
        )
        from lib.pipeline_db._core import AdvisoryLockSessionLost
        ...
        old_release_id = "10701"
        new_release_id = "10702"
        db = make_db()
        self.addCleanup(db.close)
        request_id = db.add_request(
            artist_name="Association", album_title="Old", source="request",
            mb_release_id=old_release_id,
        )
        original_conn = db.conn
        original_mark = db._mark_request_replaced

        def terminate_then_mark(*args, **kwargs):
        ...
        with (
            ...
            with self.assertRaises(AdvisoryLockSessionLost):
                db.supersede_request_mbid(
                    request_id,
                    new_mb_release_id=new_release_id,
                    new_mb_release_group_id="1070",
                    new_mb_artist_id=None,
                    new_artist_name="Association",
                    new_album_title="New",
                    new_year=None,
                    new_country=None,
                    new_tracks=[],
        ...
        # No transaction statement replayed on a replacement backend.
        current = db.get_request(request_id)
        self.assertIsNot(db.conn, original_conn)
        assert current is not None
        self.assertEqual(current["status"], "wanted")
        self.assertIsNone(db.get_request_by_release_id(new_release_id))

    def test_advisory_transaction_preserves_non_database_body_error(self):
        """A dead backend must not mask an unrelated caller body failure."""
        db = make_db()
        self.addCleanup(db.close)
