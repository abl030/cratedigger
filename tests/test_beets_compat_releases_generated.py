...
@st.composite
def release_records(draw: st.DrawFn) -> list[dict[str, object]]:
...
def assert_selected_final_window(records: list[dict[str, object]]) -> None:
...
class TestGeneratedBeetsCompatibilitySelection(unittest.TestCase):
    ...
    @example([
        {"tag_name": "v2.12.9", "published_at": "2024-08-02T00:00:00Z", "draft": False, "prerelease": False},
        {"tag_name": "v2.13.0", "published_at": "2024-08-04T00:00:00Z", "draft": False, "prerelease": False},
        {"tag_name": "v2.14.0rc1", "published_at": "2026-08-01T00:00:00Z", "draft": False, "prerelease": True},
    ])
    def test_selector_admits_only_ordered_final_window(self, records: list[dict[str, object]]) -> None:
        assert_selected_final_window(records)

    def test_known_bad_duplicate_selector_and_manifest_checker_trip(self) -> None:
        duplicate = [
            {"tag_name": "v2.13.1", "published_at": "2026-08-01T00:00:00Z", "draft": False, "prerelease": False},
            {"tag_name": "v2.13.1", "published_at": "2026-08-02T00:00:00Z", "draft": False, "prerelease": False},
        ]
