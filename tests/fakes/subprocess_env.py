...
fake commands down to a few seconds.
...
Production settles it. All four Bash entrypoints named above run from
systemd or an operator's shell with neither variable set, so a fixture that
drops them models the real world more closely than one inheriting the
reviewer's.
