...
t.section('standard mode (Pipeline tab, Wrong Matches) — the pressing switcher posts cross_pathway: false, never the opt-in');
{
  ...
  t.excludes(modal.innerHTML, 'No other pressing of this album', 'a group with a sibling carries no sole-pressing note');
...
t.section('standard mode — a release group whose only pressing on file is the current one says so instead of dead-ending (issue #1382 item 1)');
{
  ...
  for (let i = 0; i < 6; i++) await flush();
  t.contains(modal.innerHTML, 'No other pressing of this album is on file — there is nothing to switch to.',
  ...
  t.contains(renderSolePressingNote(), 'No other pressing of this album', 'the note names the situation');
}
