...
`slskd.downloadDir` must already exist and be usable by the Cratedigger identity **before the first switch** — every application unit proves it can read (and, for `cratedigger`, write) that directory at startup and refuses to start otherwise (`docs/nixos-module.md#startup-write-probe`). It is not created by any tmpfiles rule; provision it yourself or point it at the directory your `services.slskd` instance already manages. The same applies to `beets.validation.stagingDir` whenever `beets.validation.enable = true`.
