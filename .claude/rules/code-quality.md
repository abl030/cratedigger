...
- Reference layout (worked example: `search-plan advance` in PR for the cursor-advance feature):
  ...
  - `tests/web/test_routes_<module>.py` — API contract test (status-code mapping) + entry in `TestRouteContractAudit.CLASSIFIED_ROUTES` (`tests/web/test_route_audit.py`)
