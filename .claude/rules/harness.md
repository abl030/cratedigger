...
- The harness runs on the deployment-supplied Beets Python environment (`$CRATEDIGGER_BEETS_PYTHON`, from the guarded `[Beets] python` runtime contract; config via the external immutable `BEETSDIR`) — the dev shell exports its admitted test environment directly
...
- Use the `beets-docs` skill (`.claude/skills/beets-docs/SKILL.md`) to look up beets internals
