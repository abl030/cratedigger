...
- The harness runs on the cratedigger-pinned beets python env (`$CRATEDIGGER_BEETS_PYTHON`, from the module-rendered `[Beets] python`; config via `BEETSDIR`) — the dev shell exports the same env var because its python IS that env
