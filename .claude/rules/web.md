...
- **Every `tests/test_js_*.mjs` suite uses the one shared harness, `tests/js_harness.mjs`** (#1313): `const t = suite(import.meta.url);`, then `t.section(label)` headers and `t.ok/equal/notEqual/deepEqual/contains/excludes/match/notMatch/throws/rejects/fail/pass(...)`, ending in exactly one `t.done();`. Every assertion needs a message — it becomes the failure's identity (`<file>::<section>::<message>`) in the suite's failure index, and the harness fails closed on an unnamed one. Stub browser globals with `stubGlobals({document, window, fetch})` and its `restore()`, and build a `document` with `domStub(elementsById, extra?)` / `element(seed?)`, rather than hand-writing the save-assign-restore dance. Do not hand-roll `passed`/`failed` counters or import `node:assert` — `tests/test_js_suite_audit.py` rejects both
