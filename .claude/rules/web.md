...
- **A `web/js` module exports names, never a test bag** (#1313). A helper a suite needs is `export function foo(...)` at its declaration, with its JSDoc. The retired `export const __test__ = {…}` convention listed 137 names across seven modules: 56 were already named exports, listed a second time; 19 turned out to have no reader anywhere and are no longer exported at all; four it renamed on the way out, so production and tests spelled one function two ways. `tests/test_js_suite_audit.py::TestNoModuleShipsATestBag` refuses it in any spelling, on both sides. Aim to leave a helper unexported when nothing outside its module reads it, but note nothing enforces that: measured 2026-09-01, seven `web/js` exports still have no reader outside their own file, and a reader-less export is invisible until someone looks
