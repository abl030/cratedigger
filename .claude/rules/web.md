...
- **A `web/js` module exports names, never a test bag** (#1313). A helper a suite needs is `export function foo(...)` at its declaration; a helper nothing outside the module uses is not exported at all. The retired `export const __test__ = {…}` convention listed 137 names across seven modules, of which 56 were already exports and 16 were exported for nobody, and four it renamed on the way out so production and tests spelled one function two ways. `tests/test_js_suite_audit.py::TestNoModuleShipsATestBag` refuses it in any spelling, on both sides
