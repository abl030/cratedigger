...
**The same argument extends from pixels to derived operator TEXT** (#868, #882). Where the artifact is a rendered *sentence* rather than a layout, the screenshot equivalent is a **live-corpus differential**: load the pre-change renderer alongside the new one in one process, classify every real row, and report changed-row counts *by changed field*. PR #885's run - 36,303 rows, 173 changed, all in `verdict`+`summary`, `badge`/`badge_class`/`border_color` byte-identical - is the template. #882 had **five consecutive review rounds where a fix introduced a NEW falsehood while correcting an old one**, every one passing whole-repo Pyright, the full suite and adversarial review; the live corpus caught every one. Proposed as a `test-fidelity.md` rule in #888 item 3. See [[project-882-audits-shipped]].
