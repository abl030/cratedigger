...
**How to apply:** never trust a green PR check to mean the suite passes. On the
final reviewed and committed tree, run
`nix-shell --run "pyright --threads 4"` followed by
`nix-shell --run "bash scripts/run_tests.sh"` exactly once before its first
branch push. Do not replay them after an unchanged merge or during deploy.
Also: the moduleVm check (`nix build .#checks.x86_64-linux.moduleVm`) uses a
trust-auth local Postgres, so it does NOT catch DB-auth/EnvironmentFile gaps
(e.g. the youtube-ingest unit's missing pgpass injection that only surfaced
live on doc2). See [[deploy-via-master-worktree]].
