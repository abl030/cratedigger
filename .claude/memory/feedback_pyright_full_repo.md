...
When the user asks for pyright cleanup, or when Pyright is selected for
validation, run `nix-shell --run "pyright --threads 4"` on the **whole repo**.
Never scope it to the files you touched. If there are 39 errors and only 5 are
"yours", fix all 39.
...
- When running Pyright for any change in this repo, use
  `nix-shell --run "pyright --threads 4"` with no path filter. If errors appear
  anywhere — even in files you didn't touch — fix them in the same pass.
