...
- **Run `run_final_gate.sh` from a CLEAN worktree of the target revision, never the shared checkout.** The shared checkout's `.claude/worktrees/` pile (30+ stale full repo copies) sends the suite from ~150s/normal memory to 3.5 CPU-hours / 25.5G / kernel OOM — despite both pyright configs excluding the path (crawler unidentified; hunt is #1208 item 3). See [[repo-walkers-exclude-claude-worktrees]].
