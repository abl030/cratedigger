...
/**
 * Long-tail triage worklist (Pipeline sub-view) — the in-place per-row
 * action console.
 *
 * Split out of `./long_tail.js` by #522 — that module keeps the list shell
 * (band tabs, search box, row list); this module owns everything that
 * happens once a row is expanded.
 *
 * U4: the in-place action console. Selecting a row expands its
 * `.lt-detail` container into a band-aware evidence console with five
 ...
 *
 * U6: the console's secondary operator actions, plus the shared
 * post-action freshness model.
 *
 ...
 *
 * #481 item 1: the console-state consolidation. Every per-row bookkeeping
 * structure the console needs — the fetch token, the five action
 * double-fire guards, the cached YouTube resolver result, and the
 ...
 *
 * Cross-module edges with `./long_tail.js` (a deliberate, SAFE runtime-only
 * ES-module cycle — #522): this module reads `bandLabel` (the band-vs-intent
 * header), and calls back into `renderLongTail` / `loadLongTail` after a
 * row-mutating action (single-row patch vs. full-cohort refetch). Every
 * cross-reference is inside a function body, never at module top-level, so
 ...
 * removes it from the cohort. A failed fetch is non-fatal: the caller's own
 * double-fire guard (`consoleCanStart`/`consoleSettle`, e.g. the `'intent'`
 * action in {@link longTailSetIntent}) already told the operator the action
 * settled; the next manual Refresh reconciles.
 *
