...
def scenario_covers_declared_program(scenario: str | None) -> bool:
    ...
    from a stored envelope (issue #1241). Every lane that judges the same
    ``download_log`` row — the Wrong Matches cleanup reducer, a force
    import of that very row, and the auto/local import lanes — calls this,
    so they cannot disagree about whether beets proved the candidate whole.
    Unknown is ``False``: unproven means the operator's incomplete mark
    never disregards the installed side.
    """
