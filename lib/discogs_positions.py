...
manifest normalization built on it. Two production writers persist
Discogs tracklists into ``album_tracks`` — ``web/discogs.py::get_release``
(add/Replace flows) and ``album_source.py::_populate_tracks_discogs``
(the search worker's empty-manifest fallback) — and both must produce the
same rip-shaped rows, or the matcher count gate compares candidates
against a manifest no rip can equal.
...
sub-position group into a single row. Deliberate divergence from the
Beets Discogs plugin, which joins every sub-title with ``" / "``: the
first real sub-title alone is what rips name the file after, so it is
the better input for the matcher's filename-similarity check and the
search plan's track queries.
"""
...
class _PositionGroup:
    ...
    def row(self) -> dict[str, object]:
        real = [t for t in self.titles if not _is_placeholder_title(t)]
...
def normalize_release_tracks(
    ...
    '10.1'/'10.2') joins that group rather than duplicating its
    ``(disc, track)``. Raw consumers keep literal positions via
    ``web/discogs.py::get_release_raw``.
    """
    parsed: list[tuple[str, str, int | None, str, float | None]] = []
    for track in raw_tracks:
        position = str(track.get("position", "") or "")
        title = str(track.get("title", "") or "")
        duration = str(track.get("duration", "") or "")
        base, sub = split_sub_position(position)
        parsed.append((position, base, sub, title, parse_duration(duration)))

    sub_bases = {base for _, base, sub, _, _ in parsed if sub is not None}

    ordered: list[dict[str, object] | _PositionGroup] = []
    groups: dict[str, _PositionGroup] = {}
    for position, base, sub, title, length in parsed:
