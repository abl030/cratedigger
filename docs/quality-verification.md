...
- `AudioQualityMeasurement.is_cbr` is a plain boolean, not a tri-state: `True` means the files proved constant bitrate, and `False` covers both a measured VBR result and a file whose bitrate mode was never measured at all. Every reader treats `False` as VBR, the direction that never flatters a file. Authority: "thats fine, it's like, well we never proved it was cbr so it's treated as 'vbr' i think thats fine" — https://github.com/abl030/cratedigger/issues/1355#issuecomment-5526256963
