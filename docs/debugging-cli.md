...
- `pipeline-cli canonical reconcile` — Ask MusicBrainz what each release is
  called now and store any declared merge survivor in
  `album_requests.canonical_release_id` (#1059). `--id N` reconciles one
  request; omitting it sweeps every non-`replaced` row, which is what the
  daily `cratedigger-canonical-reconcile.service` does. Exit 2 unknown request,
  4 superseded row, 5 unusable stored identity. "No merge declared" and
  "already current" are answers, not failures, and exit 0.
