...
The tracked `.mcp.json` is the shared project source for main-session browser
tools. The `playwright` agent also declares the same server in its frontmatter
so specialist sessions get a fresh process. Codex TOML for both surfaces is
generated by `tools/generate-ai-adapters.py`.
...
- The tracked `.mcp.json` uses the repository-relative wrapper:
...
The shared project configuration targets the Nix/WSL development environment.
For a native Windows session, configure an equivalent user-level MCP server with
absolute Scoop paths; do not edit the tracked project source:
...
- `.mcp.json` is tracked; regenerate and commit `.codex/config.toml` after changes.
