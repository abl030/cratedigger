...
| Import-time MusicBrainz merge retag (all three non-automation importer lanes: force, local-import, YouTube's merge-follow is unreachable in practice — see #1176 PR3's `rekey_release_identity` docstring) | `lib/download_validation.py` | `_follow_merged_release`, via `validate_release_with_merge_redirect` | RELEASE, both identities, inside the importer's IMPORT session | `release_id_to_lock_key(old_release_id)` and `release_id_to_lock_key(new_release_id)`, acquired in sorted key order |
