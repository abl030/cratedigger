...
| Auto + force-import + local-import dispatch | `lib/dispatch/core.py` | `dispatch_import_core` | RELEASE | `release_id_to_lock_key(mb_release_id)` |
| Import-time MusicBrainz merge retag (all three non-automation importer lanes — force, local-import, YouTube — share this code path, but YouTube's merge-follow is unreachable in practice: see #1176 PR3's `update_request_release_for_merge` docstring, `lib/pipeline_db/requests.py`) | `lib/download_validation.py` | `_follow_merged_release`, via `validate_release_with_merge_redirect` | RELEASE, both identities, inside the importer's IMPORT session | `release_id_to_lock_key(old_release_id)` and `release_id_to_lock_key(new_release_id)`, acquired in sorted key order |
| One-album file-tag sync (#1260; three callers — the census card's web route on the server's thread-local session with no IMPORT held, `pipeline-cli sync-file-tags` relaying that route, and the merge seam best-effort inside the importer's IMPORT session, after `_follow_merged_release`'s own RELEASE pair has been released, preserving IMPORT → RELEASE order) | `lib/beets_tag_sync.py` | `sync_album_file_tags` | RELEASE, non-blocking, held across write+verify | `release_id_to_lock_key(authorized identity)` |
...
| Force-import / local-import outer (issue #1176 PR3: `dispatch_import_from_db` is the shared entry point both lanes call — see `scripts/importer.py::_execute_action_copy_dispatch`) | `lib/dispatch/entry_points.py` | `dispatch_import_from_db` | IMPORT | `request_id` |
