...
**Stated honestly (issue #1250 review finding F4, corrected by R2/R3, corrected again by V2/V3 after THAT correction pass itself introduced a false causal claim and an overreached durability claim): at the worker-death site specifically, step 1 usually narrows nothing, and the firing pattern does not show what "protects" this site.**

**Method, so this is re-runnable rather than re-quoted.** An instrumented replica of `_run_worker_death_fixture` drives the real `scripts/run_python_tests.py` subprocess exactly as production does (same env overrides, same `--jobs count`), except each generated fixture instance additionally calls `guard_refusal_reason()` directly — with the guard's actual default `--multiprocessing-fork` signature check active, the same call the real fixture makes via `guard_and_signal_parent` — and appends `{intended, live, reason}` as one JSON line to a fixed external file (outside the fixture's own ephemeral tempdir, which is deleted after each run) before performing the real guarded kill exactly as production does. For any clause-1 or clause-4 verdict specifically, also capture `/proc/<live>/comm`, `/proc/<live>/cmdline`, and `/proc/<live>/status` at that moment. Run repeatedly with `count=2`; tally `reason` across all instance records.

**Measured this session on doc1** (one sample; run-to-run counts vary with host load and are not a stable constant — re-run the method above for a fresh one rather than citing a number from here elsewhere): the large majority of instances see no refusal at all (the captured value still matches, guard proceeds); a nonzero minority hit clause 2 ("pid 1"); clause 3 ("systemd-comm") never fires. A `pid 1` verdict (clause 2) is only reachable once clause 1 has already agreed the live parent matches the captured value — so whenever clause 2 fires, the captured value was ALREADY `1`: this instance's pool worker had already been torn down, and this instance had already been reparented, before its own module-import-time capture even ran. The capture narrowed nothing in those instances; it simply recorded the post-reparenting PID as faithfully as it would have recorded the pre-reparenting one.

**Clause 1 does fire — rarely, but for real (review finding V3: an earlier version of this paragraph claimed it never does).** One measured instance: a captured value that WAS the live parent at capture time, and by the time of the live re-check the parent had already exited and this process had already been reparented — `reason` reads `"reparented: intended parent <captured-pid> is no longer this process's live parent (now 1)"`, `live=1`. That is early capture genuinely narrowing the race, caught in the act — the one case that is the design's own justification for capturing early at all, distinct from clause 2's already-1-at-capture case above.

**What clause 4 (the `--multiprocessing-fork` cmdline signature check) actually fires on (review finding V2: a first correction pass got this backwards).** That pass credited clause 4 with catching "a live parent that is neither PID 1 nor the intended pool worker — some other process adopted the orphan," citing the original pre-guard race probe above as having recorded exactly that. Measured directly instead: every clause-4 event observed has `live == intended` (no reparenting occurred at all), `comm` still `python3.14`, `cmdline` **empty**, and state `R` (running). That is the INTENDED pool worker itself, caught mid-teardown after a sibling's SIGKILL broke the pool — its address space already released, `cmdline` already cleared by the kernel, but not yet reaped — not a foreign adopter. The pre-guard race probe's "non-PID-1 `python3.14` adopter" line item is almost certainly a re-reading of this same dying-worker case, not independent corroboration of a genuine foreign adopter.

**Do not read any of this as "clauses 2 and 4 protect this site."** In this environment every refusal target observed is harmless regardless of which clause names it: PID 1 is an EPERM no-op for a non-root sender, and the intended worker mid-teardown is already dying on its own. A firing-count measurement over a population of already-harmless targets cannot demonstrate what protects anything — it only shows which clauses are exercised HERE. The clauses that would actually defend the doc1 hazard this issue is about — clause 1 catching a live reparenting event before it can matter, and clause 3 catching a `systemd --user` manager that is NOT PID 1 — are almost exactly the ones this sandboxed measurement environment cannot exercise for real: orphans here reparent straight to PID 1 (see the Action section's own ancestry check), so clause 3 is structurally unreachable here, and the one clause-1 event above is the rare exception, not the rule. That is the whole point of keeping `expected_signature` as a real, non-optional clause rather than treating it as redundant with clauses 2/3.
