...
3. **Finding #8 — force-import overrides the beets distance and nothing else.** No separate code path, no separate state, never terminal: force/manual imports run the identical post-import quality-gate/search-policy decision as automatic imports. At this review checkpoint, U7 still claimed a stage-2 proof-lock exception. That grant was superseded later the same day by [decision 21](https://github.com/abl030/cratedigger/issues/711#issuecomment-5000425284): "The verified-lossless proof lock is absolute for every import mode." See item 6 below. The `FORCE_MANUAL_SCENARIOS` gate skip is removed; the gate-runs pin restores.
