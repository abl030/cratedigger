...
| `tests/test_suite_artifact_generated.py` | `scripts/test_artifact.py` exact-target provenance checker | a suite artifact is citeable iff it completed green, started and ended clean at the expected HEAD, and ran every test in the exact discovered suite; deterministic pins exercise real concurrent allocations across synthetic Git worktrees and planted wrong-HEAD/dirty evidence |
