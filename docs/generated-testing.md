...
| `tests/test_wrong_match_policy_generated.py` | `lib.wrong_match_policy` + `FakePipelineDB.get_wrong_matches` | Wrong Matches remains a candidate/pressing review queue: all five folder/audio-integrity fact rejects plus `spectral_reject` stay excluded, while arbitrary other or NULL scenarios remain visible; the independently pinned oracle and known-bad checker prevent the shared taxonomy and fake from drifting together |
