...
| `tests/test_deploy_pin_generated.py` | `scripts/pin_nixosconfig.sh` through deterministic process-level git/nix/token seams | a retry never creates a second signed pin after the intended commit is durably reachable; every push follows the recovery-ref write and carries the token header only in environment; every started detached worktree gets a cleanup attempt across update, signature, push, and cleanup faults, with planted second-commit, push-before-ref, and missing-cleanup violations qualifying the checker |
