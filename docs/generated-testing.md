...
| `tests/test_js_ast_generated.py` | flake-pinned tree-sitter JavaScript structural audits | supported direct payload literals produce exactly the independent field oracle across identifier, shorthand, quoted/computed, nested, array, comment, string, template, escape, Unicode, and ordering worlds; spreads, dynamic keys, elisions, indirection, and methods fail closed; emitted window-handler names and direct `Object.assign(window, {...})` bindings match their authored set oracle, including escaped literal names and multiple binding blocks; planted quoted-key, template-interpolation, and missing-binding mutants qualify the checkers |
