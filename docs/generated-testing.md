...
| `tests/test_definition_default_patch_audit_generated.py` | source-local AST analysis of patched definition defaults | a test patching a dependency captured by a production callable's definition-time default must explicitly inject that dependency at the call; generated worlds vary module functions, constructors, bound/direct instance methods, positional/positional-only/keyword-only defaults, imported-name/module-attribute captures, call/import scopes, exact/aliased/qualified `unittest.mock.patch`, `patch.object`, decorators, unrelated/shadowed patch names, explicit positional/keyword/literal-dict injection, and fail-closed dynamic argument expansion; planted omission, class-resolution, provenance, and positional-binding mutants qualify the pin/property pair |
