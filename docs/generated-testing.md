...
| `tests/test_youtube_album_service_generated.py` | `lib.youtube_album_service.resolve_youtube_album` driven through the shared production Requests Session/HTTPAdapter and a loopback HTTP server | generated worlds cross every configured retryable status (429/500/502/503/504), GET/POST, absent/empty/nonempty durable cache posture, and search/seed/sibling operation site. `total=3` produces four real attempts and the public `requests.exceptions.RetryError` never escapes: outer exhaustion returns `unresolved_mirror_unavailable` without a durable write, nonempty refresh returns the exact `ok` / `from_cache` matrix, sibling exhaustion excludes only that sibling, and the existing HTTP/CLI maps remain authoritative. Escaped-exception, generic/wrong-result, fallback-drift, pre-return-write, sibling-overreach, and bypassed-attempt-count inputs qualify the checker |
...
| `tests/test_import_job_recovery_mutations_generated.py` | production recovery owner/journal CAS predicates across generated field drift | every request owner, job stage, launch-fence field, execution/child lease identity, canonical path, journal revision, and progress snapshot shown by recovery evidence remains compare-and-swap-bound; changing any one field rejects retry/close. A job-only mutant that misses the canonical-path race qualifies the checker |
