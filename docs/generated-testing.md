...
| `tests/test_completed_purge_generated.py` | `purge_completed_transfers` | foreign and nonterminal transfers are never mutated; successes require an event-stamped local path even after a prior pathless failure stamp; every owned terminal failure is durably stamped before exact-ID removal; fallback claims require slskd `requestedAt`, bind causal exact-key rows one-to-one, and preserve global transfer-ID uniqueness; summary categories are disjoint; a second pass is idempotent and every removal uses `remove=true` |
