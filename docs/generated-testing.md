...
| `tests/test_destructive_authority_generated.py` | `ban_source` + `delete_release_from_library` over identifier/lock/job-race cross-products | every identity mismatch, advisory-lock contention, and importer job that appears at the under-lock recheck causes zero request, denylist, hash, audit, beets, or filesystem mutation; a planted denylist write qualifies the checker |
