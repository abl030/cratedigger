...
| `tests/test_beets_destructive_configs_generated.py` | real pinned Beets through the production `ban_source → remove_and_reset_release → BeetsDB.locate → beet remove → post-locate` chain and exact-delete child; independently generated minimal/production plugin base, placeholder/readable/unreadable/invalid-UTF-8 secret mode, `importsource`, playlist, missing-plugin, MusicBrainz/Discogs identity, and 1/2/12-track axes | every valid configuration removes exactly the target metadata and owned files; invalid encoding, unreadable includes, and missing plugins fail before mutation; separately owned import sources plus sibling album/item/file bytes survive; exact-delete stdout is one canonical typed frame even under Python/raw-fd diagnostics. The deterministic matrix executes 48 core cells and the bounded generated property executes 18 real subprocess compositions, with known-bad stdout contamination and false-completion self-tests |
