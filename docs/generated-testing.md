...
| `tests/test_quality_lineage_generated.py` | target-quality contracts and measurement/evidence projection | bare MP3 targets fail closed unless the projected CBR/VBR mode is explicit across case/whitespace variants; exact single/equal/differing-track projections preserve mode; measurement-only early rejects never claim target policy |
| `tests/test_target_quality_contract_audit_generated.py` | lexical-scope-aware AST audit of `TargetQualityContract.from_format` calls across the canonical production-source manifest | every dynamic or literal bare-MP3 call names `projected_is_cbr`; explicit labels remain self-describing; real direct, relative, aliased, and module-qualified imports are recognized at module, function, nested-function, and class scope without leaking into sibling scopes or confusing unrelated same-named types; planted top-level and function-local omissions qualify the checker |
