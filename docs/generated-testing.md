...
| `tests/test_destructive_authority_generated.py` | `ban_source` + `delete_release_from_library` over MB, Discogs, dual-identity, identifier, lock, job-race, and real-temp-filesystem cross-products | every identity mismatch, ambiguous dual identity, advisory-lock contention, and importer job that appears at the under-lock recheck causes zero request, denylist, hash, audit, beets, or filesystem mutation; explicit MB/Discogs examples prove nominal success is reachable, and a fault-injected denylist write through the production entry point qualifies the checker |
