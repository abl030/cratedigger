...
| `tests/test_destructive_authority_generated.py` | `ban_source` + `delete_release_from_library` authority worlds plus pinned-Beets delete-manifest worlds varying track/art/sidecar presence, unknown bytes, no-op removers, and partial I/O faults | every authority rejection is zero mutation; success leaves every owned target and Beets/PG authority absent while preserving unknown bytes; cleanup failure retains Beets/PG retry authority; PG partial is explicitly album-gone/pipeline-present. Planted omitted-art, omitted-sidecar, no-op-success, unknown-overdelete, early-Beets-delete, and early-PG-delete mutants qualify the checker |
