...
| `tests/test_quality_lineage_generated.py` | target-quality contracts and measurement/evidence projection | bare MP3 targets fail closed unless the projected CBR/VBR mode is explicit across case/whitespace variants; explicit V/numeric labels own their mode and gate policy even when a caller supplies the contradictory boolean; exact single/equal/differing-track projections preserve mode; measurement-only early rejects never claim target policy |
| `tests/test_target_quality_contract_audit_generated.py` | fail-closed AST audit of `TargetQualityContract.from_format` calls across the canonical production-source manifest | every dynamic or literal bare-MP3 call names `projected_is_cbr`; explicit labels remain self-describing; canonical direct, relative, aliased, package-import, and module-qualified calls are recognized at module, function, nested-function, and class scope without leaking into sibling scopes or confusing unrelated same-named types; planted omissions, factory/class escapes, and proven-import rebindings qualify the checker |
...
| `tests/test_completed_purge_generated.py` | `purge_completed_transfers` | foreign and nonterminal transfers are never mutated; successes require an event-stamped local path even after a prior pathless failure stamp; every owned terminal failure is durably stamped before exact-ID removal; fallback claims require a real slskd datetime-shaped `requestedAt`, bind causal exact-key rows one-to-one, and preserve global transfer-ID uniqueness; terminal categories conserve every row through successful removals, false returns, and exceptions (`removed + removal_failed + success_waiting + failure_unconfirmed + foreign`) while failed removals remain resident and `failure_stamped` remains an overlapping action metric; a second pass is idempotent and every removal uses `remove=true` |
...
| `tests/test_definition_default_patch_audit_generated.py` | source-local AST analysis of patched definition defaults | a test patching a dependency captured by a production callable's definition-time default must explicitly inject that dependency at the call; generated worlds vary module functions, constructors, instance/class/static descriptors through class and instance access, positional/positional-only/keyword-only defaults, imported-name/module-attribute captures, absolute/relative import replacement across alias/instance/local-function maps with every star import rejected, call/import scopes, exact/aliased/qualified `unittest.mock.patch`, `patch.object`, function and class decorators, unrelated/shadowed patch names, sibling-chain nested helpers with definition-time decorator metadata and call-site patch state, immediate function defaults/decorator expressions/eager annotations and lambda defaults, deferred/future annotations, class decorator/base/keyword/body construction timing and post-construction class-patch application using sequential constant direct/aliased/qualified `patch.TEST_PREFIX` state, dynamic mutation rejection, independent test/method scope restoration, and direct-local-helper propagation, escaping callback and lambda bodies, ordinary/annotated/walrus patch-alias assignment with helper overwrites, function/class declaration shadowing, local-call callable/argument/body order with one-pass walrus alias swaps, sequential sync/async `with` context evaluation, non-assignment/comprehension binders, explicit positional/keyword/literal-dict injection, and fail-closed dynamic argument expansion; planted omission, class/descriptor resolution, import invalidation/star rejection, test-prefix selection/scope restoration, provenance, positional binding, nested registry/decorator timing, definition-expression traversal/lambda deferral, class-body/class-decorator timing, assignment provenance, declaration binding, argument-before-body/single-traversal, context traversal/activation, escaping-boundary, and binder-invalidation mutants qualify the pin/property pair |
