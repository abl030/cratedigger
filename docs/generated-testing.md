...
| `tests/test_terminal_outcomes_generated.py` | DB-owned import-success, importer-rejection, and preview-measurement-failure transactions across all four import job types | every injected request/audit/denylist/final-request/job boundary failure rolls the entire terminal bundle back; committed bundles add exactly one mandatory audit and finalize the job; Replace-first races freeze the ancestor; planted partial-audit and missing-job mutations qualify the checker |
