...
| `tests/test_beets_destructive_configs_generated.py` | real pinned Beets subprocesses through both production destructive entry points across minimal, full module-rendered plugin, readable/unreadable include, `importsource.suggest_removal`, and automatic-playlist profiles; MusicBrainz/Discogs identities; 1/2/12 tracks; sibling pressings | every valid common configuration deletes exactly the authorized album metadata and owned files while preserving its sibling; an unreadable declared include fails before mutation; exact-delete stdout remains one canonical typed frame even when Python and raw-fd plugin diagnostics are injected. The deterministic matrix executes all 36 cells; generated sampling replays the same real observations, with known-bad stdout contamination and false-completion self-tests |
