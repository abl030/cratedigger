...
| `tests/test_spectral_attempt_audit_generated.py` | both attempt-audit adapters, `process_claimed_preview_job`, and the real PostgreSQL evidence writer | candidate and installed-HAVE measurement are independently once per content snapshot across automation and force jobs: matching usable snapshots project their complete persisted spectral facts without another scan even when the other side changes, while changed, absent-grade, error-grade, or unrecognized ordinary HAVE evidence runs the required measurement and exact-FK CAS persistence; qualified checkers reject both redundant scans and skipped required scans |
...
installed HAVE remains independently authorized through its exact Beets
identity and byte-exact content snapshot: a matching complete row projects its persisted
spectral fact even while a new/changed candidate is measured, while missing,
unusable, or changed HAVE evidence runs the analyzer and persists the result
before import. Same-size byte replacements are generated explicitly and must
miss reuse on both sides. The explicit unchanged force examples use the
Rolling Stones and So Why? incident identities.
