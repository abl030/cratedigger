...
| `tests/test_completed_purge_generated.py` | `purge_completed_transfers` | foreign and nonterminal transfers are never mutated; unstamped successes remain event/local-path gated; every owned terminal failure is durably stamped before exact-ID removal; T1.5 race fallback claims causal exact-key open rows one-to-one; a second pass is idempotent and every removal uses `remove=true` |
