...
| `tests/test_definition_default_patch_audit_generated.py` | source-local AST analysis of patched definition defaults | a test patching a dependency captured by a production callable's definition-time default must explicitly inject that dependency at the call; generated worlds vary imported-name and module-attribute defaults, direct and aliased calls, explicit injection, and unrelated patches/calls; a planted omitted-injection mutant qualifies both the pin and property |
