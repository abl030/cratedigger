...
| `tests/test_target_quality_contract_audit_generated.py` | fail-closed canonical-import/direct-call audit of `TargetQualityContract.from_format` across the production-source manifest | every dynamic or literal bare-MP3 call names `projected_is_cbr`; explicit labels remain self-describing; canonical direct, relative, aliased, package, `evidence_types`, and module-qualified calls are recognized across lexical scopes; peer package imports are order-independent; target star imports, computed factories, class/module/factory escapes, and proven-import rebindings qualify the checker |
