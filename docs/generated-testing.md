...
| `tests/test_target_quality_contract_audit_generated.py` | source-local AST audit of `TargetQualityContract.from_format` calls across the canonical production-source manifest | every dynamic or literal bare-MP3 call names `projected_is_cbr`; explicit labels remain self-describing; real direct, relative, aliased, and module-qualified imports are recognized without confusing unrelated same-named types; a planted omission qualifies the checker |
