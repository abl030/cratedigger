...
| `tests/test_dispatch_outcomes_generated.py` | `dispatch_import_core` + every current rejection writer (`_reject_import_from_evidence_decision`, `_record_rejection_and_maybe_requeue`, `DatabaseSource.reject_and_requeue`, and `_reject_request_auto_import`) + the job-less success writer (`_do_mark_done`, driven directly) + the job-less quality-gate policy writer (`_check_quality_gate_core`, driven directly) | the auditability law (every outcome writes a download_log row); full routing oracle vs `dispatch_action` across the decision universe + no-JSON crash path; U11 lifecycle policy for all 5 preimport facts (including `mixed_source`, missing from the hand-written table): automation self-heals to `wanted`, while terminal persistence preserves current operator-owned search state; every rejection writer preserves the `ValidationResult` distance/scenario projection exactly, including NULL and measured 0.0; for the four rejection writers plus the job-less quality-gate writer, every `source_denylist` row is traceable to one of the three terminal-outcome bundles (job-backed `persist_import_terminal_outcome`, job-less `persist_request_rejection_outcome`, or job-less `persist_request_policy_outcome`) rather than a separate `db.add_denylist` call outside all three (issue #1355 items 3 and A2); the job-less success writer commits its transition and audit row through `persist_request_success_outcome` and never falls back to a separate `log_download` call (issue #1355 item A1) |
...
| `tests/test_deploy_generated.py` | `scripts/deploy.sh`'s nixos-upgrade wait loop, the real script driven over generated `systemctl show` snapshot streams through the real-git deploy fixture in already-pinned mode | the loop's decision agrees with a pure oracle on every stream: waiting while the previous invocation is still shown, success only on the tracked invocation reaching inactive/dead/success, and a named failure (disappeared, changed mid-deploy, failed, unexpected state) otherwise, with a stream that never resolves timing out rather than passing; the oracle's own clauses are self-tested known-bad |
