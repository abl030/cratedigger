...
| `tests/test_definition_default_patch_audit_generated.py` | canonical source-local grammar for patched definition defaults | a direct literal `patch(...)` or imported-owner `patch.object(...)` covering a directly imported production callable must inject the captured dependency by explicit keyword; generated worlds share the deterministic tier's authored model and vary local-name/module-attribute defaults, direct/module callable imports, direct/qualified patch APIs, string/object targets, decorators/with-items, injection, and unrelated patches; dynamic targets, manual patchers, callable aliases, nested execution, positional-only or dynamic argument injection, and conflicting imports fail closed with path/line diagnostics instead of extending a Python evaluator; planted omitted-finding and unrelated-false-finding verdicts qualify the pin/property pair |
