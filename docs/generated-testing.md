...
| `tests/test_evidence_media_identity_generated.py` | temporary Beets-shaped SQLite through `BeetsDB.get_album_info` → current-evidence propagation → final policy projection | every admitted Beets format normalization output has one shared evidence-policy meaning: OGG/Vorbis and OGG/Opus preserve a lossless-source spectral fact, M4A/AAC preserves it, and native M4A/ALAC or FLAC remains generation-strict. The checker is separately qualified by a known-bad ALAC-preserved result; removing Vorbis, conflating M4A with AAC, or admitting native lossless kills the outer-adapter contract. |
...
| `tests/test_lossless_lineage_check_generated.py` | migration 073's real PostgreSQL lineage schema plus same-address FakePipelineDB writer-order worlds | installed spectral measurements and durable conversion lineage may coexist; stale same-key writers preserve both in either order; the exact manifest-aware R19 predicate alone determines whether a carried source spectral fact is reusable, failing closed for native, mixed, or unresolved output |
