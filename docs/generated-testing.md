...
|--------|--------|------------|
| `tests/test_convergence_pipeline_db_generated.py` | the real migration-070 PostgreSQL request-local derivation and atomic stop CAS, differentially checked against `derive_convergence_signal` | generated direct/cross-walk attribution, atomic comma-joined peer sets, timestamp ties, null/different cliff breaks, raw within-band spread, codec diversity, and ineligible worlds produce the same complete signal facts as the independent reference; separately committed late evidence links invalidate the opaque snapshot token before mutation; raw-string peer counting and cross-walk admission mutants qualify the checker |
