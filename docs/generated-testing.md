...
| `tests/test_jellyfin_refresh_generated.py` | the real `trigger_jellyfin_scan` entry point with only `urllib` replaced at the network leaf | every configured target submits one exact metadata-safe Jellyfin 10.11 refresh (Default metadata/images, both replace-all flags false, no explicit recursive flag), with POST/token/timeout intact and no broad fallback; HTTP 204 remains queueing evidence only, while transport/HTTP/runtime failures stay inside the best-effort notifier boundary |
