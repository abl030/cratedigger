...
| `tests/test_destructive_authority_generated.py` | `ban_source` + `delete_release_from_library` over MB, Discogs, same-ID dual-layout, distinct dual-identity, malformed-nonempty, empty/whitespace/zero-sentinel, identifier, lock, job-race, and real-temp-filesystem cross-products | every identity mismatch, ambiguous/invalid server identity, advisory-lock contention, and importer job that appears at the under-lock recheck causes zero request, denylist, hash, audit, beets, or filesystem mutation; explicit MB/Discogs/sentinel examples prove nominal success is reachable, and a fault-injected denylist write through the production entry point qualifies the checker |
