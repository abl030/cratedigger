...
| `tests/test_current_library_display_generated.py` | the production exact-identity Library artist and detail merges over generated current-Beets, acquisition-history, evidence, request-status, dual-tag attachment, request-cardinality, and malformed-authority worlds | presence, Captured history, current quality/proof, and tracking remain independent; zero matching requests keeps the primary observed identity untracked, one exact match owns the artist/detail action key, source, and detail history, while cross-source, duplicate modern Discogs, and modern-plus-legacy Discogs candidates fail closed as ambiguous instead of hiding a request; malformed, conflicting, and identityless pipeline-only rows remain visible but have no actionable release ID/source and never attach or suppress; sibling retags and deletions produce Captured plus Missing and held plus Untracked without identity inference, and planted presence-to-capture, first-observation action-key, detached-source, detached-detail-history, first-wins overwrite, and permissive-identity mutants are rejected |
