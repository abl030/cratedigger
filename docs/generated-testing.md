...
| `tests/test_current_library_display_generated.py` | the production exact-identity Library merge over generated current-Beets, acquisition-history, evidence, and request-status worlds | presence, Captured history, current quality/proof, and tracking remain independent; sibling retags and deletions produce Captured plus Missing and held plus Untracked without identity inference, and a planted presence-to-capture inference mutant is rejected |
