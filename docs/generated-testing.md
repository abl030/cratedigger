...
| `tests/test_destructive_authority_generated.py` | `ban_source` + `delete_release_from_library` authority worlds plus pinned-Beets delete-manifest worlds varying track/art/sidecar presence, unknown bytes, no-op removers, partial I/O faults, and lost subprocess/protocol acknowledgements | every authority rejection is zero mutation; success leaves every owned target and Beets/PG authority absent while preserving unknown bytes; cleanup failure retains Beets/PG retry authority; PG partial is explicitly album-gone/pipeline-present; only an ambiguous child acknowledgement plus authoritative exact-album-and-item absence and a retained former path may reconcile as completion, after which optional PG purge remains last and notification runs once. Planted omitted-art, omitted-sidecar, no-op-success, unknown-overdelete, early-Beets-delete, early-PG-delete, blind-failure reconciliation, orphan-item acceptance, and stranded-commit mutants qualify the checkers |
...
for Hypothesis to explore. The underlying delete crash worlds and notifier
