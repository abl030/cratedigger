...
| `tests/test_import_job_payload_generated.py` | `ImportJob.from_row` strict job-discriminated payload decoding | every generated valid force, automation, and YouTube input payload decodes to its exact Struct; arbitrary planted unknown fields fail at the database boundary; the discriminator/payload-class checker rejects a known-bad cross-job Struct |
