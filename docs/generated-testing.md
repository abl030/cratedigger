...
| `tests/test_target_quality_contract_audit_generated.py` | source-local AST audit of `TargetQualityContract.from_format` production calls | every dynamic or literal bare-MP3 call names `projected_is_cbr`; explicit labels remain self-describing; direct, aliased, and module-qualified imports are recognized; a planted omission qualifies the checker |
