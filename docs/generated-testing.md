...
| `tests/test_definition_default_patch_audit_generated.py` | conservative source-local grammar for patched definition defaults | an active patch over a directly called production callable must inject its captured dependency by explicit keyword; shared deterministic/generated worlds vary local/module/wrapper/tuple defaults, functions/constructors/assigned instances, direct/module/conflicting/shadowed call bindings, direct/qualified/`unittest.mock` APIs, string/object/dict/dict-values/multiple/dynamic targets, decorators/with-items/later contexts, injection, and whether the captured callable executes; literal string patches and direct-import-owner `patch.object` are the supported grammar, while relevant uncertainty fails closed only after a direct omitted call is found, so explicit injection and unrelated use remain accepted; planted omitted-finding and unrelated-false-finding verdicts qualify the pin/property pair |
