...
- `cratedigger-canonical-reconcile.service` — oneshot, `restartIfChanged = false`, `TimeoutStartSec=1h`, runs as `cfg.user`. Wraps `scripts/run_canonical_reconciliation.py` via the `cratedigger-canonical-reconcile` wrapper bin, which passes both `--dsn` and `--config` so the job reads the deployment-owned `musicbrainz.apiBase`. Asks the MusicBrainz mirror what each non-`replaced` request's release is called now and stores any declared merge survivor in `album_requests.canonical_release_id` (#1059). `wants = ["cratedigger-db-migrate.service"]` (not `requires` — see the migrate unit's entry above) and gates on schema currency itself. **No slskd health gate**: this job never talks to slskd, and it deliberately does NOT live inside `cratedigger-unfindable.service`, whose slskd gate would silently stop merge reconciliation for the length of any slskd outage. Sweeps whatever `musicbrainz.apiBase` points at; against public MusicBrainz that is slow for the same reason every other mirror consumer is slow without one, which is the operator's call to make.
- `cratedigger-canonical-reconcile.timer` — `OnCalendar=daily`, `Persistent=true`, `RandomizedDelaySec=30min`. Whole library, every day: ~8,500 rows at ~72ms against the local mirror is about ten minutes. Deliberately not miss-triggered — merge drift is invisible from the read side until `mbsync` retags, and invisible from the write side until a download fails to match, so waiting for either signal means learning about a merge only after it has already cost something.
