...
| `beets.config.{directory,library}` | production values | The single shipped Beets root/SQLite pair. Both values are rendered into module-owned `config.yaml` and `[Beets]` in `config.ini`; every production reader opens them together. `beets.directory` follows `beets.config.directory` by default. |
