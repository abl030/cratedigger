...
1. **Trigger**: `_timeout_album()` (download.py), `reject_and_requeue()` (album_source.py), and the `_enqueue_completed_processing()` materialize-grace-expired reset (`download.py`, issue #822 item 4 — the residual materialize-failure path with no other cooldown coverage) all call `db.check_and_apply_cooldown(username)` after logging the outcome. That reset's row records the machine reason in `beets_detail` (issue #868). The full vocabulary that can land there: `event_path_never_stamped`, `event_path_gone_from_disk`, `unsafe_source_path`, `source_open_failed_<ERRNO>`, `source_read_failed_<ERRNO>`, `source_preflight_refused`, `slskd_root_unsafe`, `slskd_root_missing`, `slskd_root_open_failed_<ERRNO>`, `slskd_root_read_failed_<ERRNO>`, `slskd_root_refused`, `processing_authority_unsafe`, `processing_path_missing`, `processing_open_failed_<ERRNO>`, `processing_read_failed_<ERRNO>`, `processing_write_failed_<ERRNO>`, `materialize_authority_failed`, `private_materialize_failed`, `staged_path_missing`, `staged_path_missing_tracked_files`, `empty_manifest`, `duplicate_final_basename`, and `abandoned_interrupted_auto_import`. The cooldown is applied identically whichever reason it was — the reason is evidence, never a lifecycle input. `lib/failure_presentation.py` turns each of them into operator copy at render time; the raw token stays in the column.
