...
- `search_log_id INTEGER NULL REFERENCES search_log(id) ON DELETE SET NULL` — migration 086 (issue #811): the exact `search_log` row whose `found` outcome produced the grab this row audits. This is the ONLY link between "what the search found" and "what came of it"; before it, a `found` search row and the `timeout` it produced 20 seconds later were two unconnected facts about the same peer. Partial index `idx_download_log_search_log_id` covers the non-NULL rows. `ON DELETE SET NULL`, not CASCADE: `download_log` is the operator's audit trail and `search_log` is forensics about how the row came to be, so if a search row ever goes away the audit row must survive without it. Nothing deletes `search_log` rows today except the request-level cascade (which takes the `download_log` rows with it anyway), so this is the fail-safe choice for a future writer rather than a live path.
  ...
  - **One writer has no `DownloadInfo` and carries the link anyway.** An AUTOMATION preview measurement failure is a grab outcome, but `_record_preview_measurement_failed` never sees a `DownloadInfo`. Under invariant 10 the owner's `active_download_state` is still attached (the terminal bundle clears it last), so `lib/dispatch/outcome_actions.py::_automation_grab_search_link` reads the link straight off the owned request. It is scoped to the automation lane by its one call site — force and local previews measure a folder already on disk, and `scripts/import_preview_worker.py`'s KTD1 forbids the YouTube path from reading that column at all — and is fail-soft, so an audit-enrichment read can never turn a measurement failure into a crash.
  - **What stays NULL, and why that is correct.** The enqueue-time `user_offline` row (`lib/enqueue.py`, written before any search row exists); the merge audit (`lib/download_validation.py`) and library-delete audit (`lib/destructive_release_service.py`), neither of which is a grab; `insert_youtube_running` queue rows; force-import, local-import and manifest-guard rows, whose `DownloadInfo` is built from candidate evidence or a bare username rather than from live grab state; a force/local/YouTube preview measurement failure (per the automation-only scoping above); the non-automation import-job failure diagnostic (`import_job_failure_outcome`, whose payload names an ORIGIN `download_log` row — inheriting that row's link would attribute an operator action to a search that did not cause it); and every row written before migration 086.
...
search that never enqueued or predates migration 086. The `search_log`
...
ACCEPTANCE (all history when there has never been one).

Two closed outcome vocabularies decide that, both in
`lib/pipeline_db/download_log.py`:

- `IMPORT_ACCEPTANCE_OUTCOMES` = `success`, `force_import`, `local_import`,
  `manual_import` — the window boundary. A force/local/manual import is as
  much "we got a copy" as an automation success; scoping the window to
  `success` alone reported "all history" for 1,112 live requests whose only
  acceptance is one of the other three, and opened the window too early for
  367 more whose newest acceptance sat above an older `success` (measured
  2026-09-09, issue #811 review). Its writable subset,
  `WRITABLE_IMPORT_ACCEPTANCE_OUTCOMES`, is what
  `lib/terminal_outcomes.py`'s successful-acceptance refusal guard admits —
  deliberately narrower, since `manual_import` has no writer and admitting
  it there would be dead permission on a safety check. The difference
  between the two is pinned by
  `tests/test_pipeline_db.py::TestSharedOutcomeVocabularies`.
- `GRAB_OUTCOMES` = `success`, `rejected`, `failed`, `timeout`,
  `measurement_failed`, `have_analysis_error` — what `grabs`/`grabs_total`
  count. Positively enumerated, because the older
  `source='slskd' AND soulseek_username IS NOT NULL` shape also counted
  rows no transfer produced: `force_import` (1,927 live rows, an operator
  re-importing a folder already on disk), `curator_ban` (8) and
