...
`import_one.py` emits an `ImportResult` JSON blob (`__IMPORT_RESULT__` sentinel on stdout). Contains: decision, conversion details, V0 probe evidence, quality comparison (new vs prev bitrate), postflight verification (beets_id, path), and an attempt-local `spectral` audit. Preview analyzes the incoming candidate before conversion, preserving its album grade, spectral floor, suspect percentage, per-track grade/HF deficit/cliff detail, or error. `HAVE` is projected from the installed release's durable source evidence (`album_requests.current_evidence_id`, with the legacy request spectral fields as fallback); the analyzer is never run over a post-import Opus or other derivative because that would rewrite source provenance. These audit fields are display-only and deliberately separate from `new_measurement` / `existing_measurement`. Every import path (success, downgrade, transcode, provisional, suspect-lossless rejection, error, timeout, crash) logs to download_log.
