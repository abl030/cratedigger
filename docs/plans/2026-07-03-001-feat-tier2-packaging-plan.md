...
- **Approach:** `beets_subprocess_env()` sets `BEETSDIR` from `CratediggerConfig` (new config.ini key rendered by the module) — the `HOME=/home/abl030` literal is deleted. `_beet_binary()` reads the configured binary path (module renders the pinned beet's store path); the `/etc/profiles/per-user/abl030` fallback is deleted. `run_beets_harness.sh` collapses to exec'ing the pinned interpreter with the beets env's site-packages (provided via env vars from the module wrapper) — all `.beet-wrapped` grep-scraping deleted. Every `beets_subprocess_env` callsite moves in this one unit — the four direct ones (`lib/dispatch/subprocess_runner.py`, `lib/beets_album_op.py`, `lib/beets.py`, `harness/import_one.py`) plus the indirect reach-throughs (ban-source via `web/routes/pipeline.py` and `remove_and_reset_release`, which route through `lib/beets_album_op.py`); the grep-guard test makes the set mechanical rather than a maintained list. `${src}` must never land on the beets subprocess PYTHONPATH (`lib/beets.py` shadow hazard).
