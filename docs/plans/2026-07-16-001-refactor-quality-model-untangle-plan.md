...
- **Product authority:** The [issue #711 thread](https://github.com/abl030/cratedigger/issues/711) and its operator decisions; this plan translates that authority and cannot widen it. In particular: Authority: "D19 — Force-import overrides the beets distance and nothing else." — [decision 19](https://github.com/abl030/cratedigger/issues/711#issuecomment-4999204451). Authority: "The verified-lossless proof lock is absolute for every import mode." — [decision 21](https://github.com/abl030/cratedigger/issues/711#issuecomment-5000425284). Product Contract unchanged from the requirements pass except Outstanding Questions, which planning resolved from implementation research (the spectral-None audit expectation was refuted — see KTD1).
...
Restructure the quality model so acceptance, upgrade ceiling, installed-bytes authority, source lineage, materialized format, and search intent each have exactly one owner. Verified-lossless proof becomes the only acquisition stop, absolute across every import mode; every other state keeps searching. Evidence facts adopt a two-axis vocabulary — subject (`installed` | `source`) and provenance (`measured` | `carried`) — and Vorbis/WMA join the codec rank model with UNKNOWN as the orderable bottom.
...
1. **Verified-lossless proof is the sole, absolute terminal boundary.** No import — automatic, force, or manual; lossy or lossless — crosses it in either direction; there is nothing left to acquire. Operator Replace/re-request is the only way back in and is never blocked by proof.
...
6. **Acquisition facts carry unconditionally across evidence rebuilds; the scalar seam closes.** Every member of the [canonical acquisition-fact set](../quality-verification.md#evidence-addressing-propagation-and-ownership) describes the acquisition event and cannot be re-derived from converted files. No audio-identity precondition guards the carry — out-of-band file replacement is outside the state model.
...
19. **Force-import overrides the beets distance and nothing else** (session-settled: user-directed, 2026-07-17). Force/manual imports take the identical post-import quality-gate/search-policy path as automatic imports — no separate code path, no separate state, never terminal. (The "force-import is not blocked by the proof lock" exception originally noted here was superseded the same day by [decision 21](https://github.com/abl030/cratedigger/issues/711#issuecomment-5000425284): "The verified-lossless proof lock is absolute for every import mode.")
...
21. **The verified-lossless proof lock is absolute for every import mode** (session-settled: user-directed, 2026-07-17). Force/manual imports hit `verified_lossless_locked` exactly like automatic candidates — force-import bypasses the beets distance and nothing else (decision 19's principle completed). This restores decision 1's letter ("Operator **Replace/re-request** is the only way back in"): the force-import bypass in KTD4/U7 was planning-time widening the operator never directed. Wrong-Matches cleanup semantics are unchanged: the disk-loaded verified-parent short-circuit stays, and a backfilled parent proof reaching the force-mode reducer now deletes via the locked decision — carried proof remains decisive through the audited path (out-of-band audio mutation is outside the state model per decision 6).

### Requirements

**Terminal boundary and search policy**

- R1. Verified-lossless proof completes acquisition terminally; no candidate in any import mode, lossy or lossless, may replace a proof-bearing HAVE.
...
- R6. Every lossy import in any import mode that keeps the request `wanted` denylists its winning source at import time.
...
- R19. On any evidence rebuild, every on-disk fact is freshly re-measured; every member of the [canonical acquisition-fact set](../quality-verification.md#evidence-addressing-propagation-and-ownership) copies across unconditionally with provenance `carried`.
...
- R23. One operator/agent deploy one-shot materializes every request-scalar-carried member of the [canonical acquisition-fact set](../quality-verification.md#evidence-addressing-propagation-and-ownership) onto a linked evidence row for every non-replaced request; requests whose album can no longer be resolved on disk surface to the operator rather than keeping a silent scalar fallback alive.
...
- AE1. **Covers R1.** Given a HAVE with verified-lossless proof, when any candidate completes in any import mode — including a genuine FLAC — then no replacement occurs and the request stays `imported`.
...
- AE7. **Covers R19, R22.** Given a size-changing retag flips the snapshot fingerprint, the rebuild re-measures every on-disk fact and carries every member of the [canonical acquisition-fact set](../quality-verification.md#evidence-addressing-propagation-and-ownership) with provenance `carried`; the next suspect lossless candidate still hits the anchor comparison, not "no comparable probe".
...
Suggested PR series, each independently deployable: **PR1** U1+U2+U3 (analysis failure, migration first); **PR2** U4 (affirmative verification); **PR3** U5+U6 (policy rewrite + config retirement — nixosconfig wrapper edit rides this deploy window); **PR4** U7 (proof boundary); **PR5** U8+U9 (codec model); **PR6** U10+U11 (evidence vocabulary + seam close — strict intra-window order: deploy the new generation with its timers declaratively held, or complete a backwards-compatible one-shot before the switch; a runtime pre-switch mask is not sufficient; the recorded wanted∧verified reconciliation gates resuming cycles). Migration-before-code holds within every PR: the migration must be live on doc2 before code reading new columns runs (deploys apply migrations before workers start; `cratedigger.service` gates on schema currency).
...
- **Goal:** A proof-bearing HAVE blocks every candidate in every import mode, in both directions, with a named, non-punitive decision.
...
- **Approach:** KTD4: new stage-2 decision `verified_lossless_locked`, checked before quality comparison when current evidence carries proof: no import, no denylist, no narrowing change, request stays `imported`. Authority: "The verified-lossless proof lock is absolute for every import mode." — [decision 21](https://github.com/abl030/cratedigger/issues/711#issuecomment-5000425284). Force/manual imports hit it too; the original "force-import bypasses it" clause here was planning-time widening, superseded. Replace and re-request remain the operator's way back in — they supersede the request row rather than importing over proof. Both twins gain the branch in lockstep (parity contract).
- **Execution note:** invert the family-1/5 pins RED first.
- **Test scenarios:** flips: `test_genuine_flac_reimports_verified` (`tests/test_simulator_scenarios.py:1343-1350`) and `test_mp3_higher_than_lofi_imports` (`:1359-1363`) → named negative pins asserting the lock. Protected: every verified-completion scenario. (The original "force-import must still import over proof" guard here was superseded by decision 21 — the guard now asserts the opposite: force-import is locked too.) New: generated property — for any current evidence with proof, no candidate world in ANY import mode produces a replacement (both lossy and lossless candidate strategies); known-bad self-test plants a proof-ignoring mutant.
...
- **Test scenarios:** real-PG round-trip asserting **every** field of a v4 evidence row (subject/provenance markers, proof provenance) reads back — Rule A; msgspec RED test feeding a legacy lineage value into the new struct asserts `ValidationError`; cross-product CHECK known-bad (installed+carried rejected); AE7 slice — fingerprint flip rebuild re-measures on-disk facts and carries every member of the [canonical acquisition-fact set](../quality-verification.md#evidence-addressing-propagation-and-ownership) with provenance `carried` (extends `test_evidence_generated.py`'s existing carry properties); v3-row rebuild-on-touch slice (old row triggers rebuild, acquisition facts survive, markers appear); the `6cf26a4`-descended converted-evidence property still green; twin parity property still green.
...
- **Approach:** Delete the three named legacy readers and the request-stamp spectral read inside `evidence_from_album_info` (all colocated, `lib/quality_evidence.py:397-462, 741-751, 773`), plus the two structurally identical reads research found: `lib/measurement.py:817` (`current_spectral_grade` fallback) and `lib/import_preview.py:280-288` (`load_persisted_existing_spectral`'s legacy branch). Stamp *writers* stay (decision 16); rendering/audit readers stay. Deploy one-shot (operator/agent-run, never committed) under the [strict hold](../../.claude/skills/deploy/SKILL.md#holding-timer-driven-work-across-a-switch): deploy the reader-deletion generation with its timers declaratively held, or complete a backwards-compatible one-shot under the pre-switch mask; do not make the precondition depend on an imperative post-switch verification window. Before the new code's first cycle, for every non-replaced request whose scalars carry any member of the [canonical acquisition-fact set](../quality-verification.md#evidence-addressing-propagation-and-ownership), ensure a linked evidence row exists and materialize those facts with provenance `carried`; requests whose album can no longer be resolved on disk print to the operator (the #723 blank-path one-shot is the template). **2026-07-18 correction (#737):** the earlier "proof or anchor" shorthand omitted source-subject spectral and was not a complete R23 reconciliation. The recorded wanted∧verified reconciliation gates resuming cycles; post-one-shot, it is a one-row glance.
