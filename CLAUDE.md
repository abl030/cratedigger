...
- **Beets** — deployment owns the compatible package, immutable effective config, SQLite catalog, files, state, secrets, current identity/path/tags, and plain operator `beet`; Cratedigger owns exact acquisition requests/history/proof. Its only mutation lanes are the serial JSON importer harness, the explicit exact-album delete child, and the import-time merge retag (`beet modify -a -M -W -y`, one album pinned by primary key AND identity value, identity only, never moves files, reached by both the automation and force import lanes; #1087 replaced the original `beet mbsync -M` primitive, which cannot follow a release-only merge; #1093 replaced the query's anchored regex with an exact SQL-equality match, the same mechanism the post-retag guard already reads with, then ANDed in an `id:=<album_id>` clause pinning the exact row the guard resolved, closing the residual time-of-check/time-of-use gap a value-only re-select left open); all other access is observational. Never raw-import or use `remove -d`; always match `candidate_id`, never `candidate_index`. `docs/beets-primer.md`.
...
- **slskd client + event ingestion** — all HTTP uses `lib/slskd_client.py`; the event feed is the ONLY completed-file location authority (unstamped materialization hard-fails then self-heals to `wanted`). Delete only event-stamped paths with empty-dir pruning, never inferred-folder rmtree. **Good-citizen ownership (#571): destroy only positively proven Cratedigger state/files; shared slskd is safe.** Accepted transfer-ledger rows authorize cancellation only for their exact `(username, filename)` queue key when no `downloading` row backs it; pending/foreign keys are never cancelled. Disk reaping after 7 days requires an event `local_path`; unowned files are never deleted. Protect `failed_imports/`, `wrong_matches/`, and active downloading paths regardless. Searches and enqueue intents are write-ahead ledgered; only accepted enqueue promotes destructive ownership, and completion events cannot promote rejected/unknown intent. Terminal `Completed,*` records are purged by confirmed queue key/current ID; transfer cleanup alone never authorizes disk deletion. Pending intents age out; accepted evidence retention exceeds the reaper threshold, so pruning makes files unowned, not reapable. `lib/slskd_events.py`, `lib/slskd_transfers.py`, `lib/slskd_searches.py`, `lib/slskd_transfer_ledger.py`.
...
The importer queue is the automatic Beets-mutation boundary: web/CLI/poller enqueue; `cratedigger-importer` drains serially under an advisory singleton lock. A request-backed automation job owns its request for the complete `processing` lifetime. Any world failure, including a launched-but-unacknowledged child, records audit evidence and returns the request to `wanted` for re-derivation. The other two Cratedigger mutation lanes are the explicit exact-album delete child and the import-time MusicBrainz merge retag (`beet modify -a -M -W -y`, pinned by the guard-resolved primary key AND the identity value, identity only, never moves files, both `RELEASE` locks held, under whichever exact import claim the caller took — the automation processing owner or a claimed `force_import` job, #1080 — `lib/beets_retag.py`; #1087 replaced the original `beet mbsync -M` primitive, which cannot follow a release-only merge; #1093 unified the query's selection mechanism with the post-retag guard's and closed the residual time-of-check/time-of-use gap); add no fourth lane. Authority: "this was supposed to be just at import time. we go, oh, re=direct. re-tag and then import." — https://github.com/abl030/cratedigger/issues/1059. Plain operator `beet` remains external authority and is not serialized by Cratedigger.
...
1. **NEVER `beet remove -d`** — deletes files permanently. Explicit Bad Rip, Replace, and library-delete actions use the admitted exact-album child in `lib/beets_delete.py`; selector-based deletion is retired.
