...
- **Beets** — deployment owns the compatible package, immutable effective config, SQLite catalog, files, state, secrets, current identity/path/tags, and plain operator `beet`; Cratedigger owns exact acquisition requests/history/proof. Its only mutation lanes are the serial JSON importer harness, the explicit exact-album delete child, and the import-time merge retag (`beet modify -a -M -W -y`, identity only, never moves files, reached by both the automation and force import lanes; see Decision architecture for the exact primary-key-plus-identity selection guard, #1087, #1093); all other access is observational (never raw-import or `remove -d` — Critical rules 1-3). `docs/beets-primer.md`.
