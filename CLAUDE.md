...
- **slskd client + event ingestion** (issue #146) — all slskd HTTP calls go through the in-repo typed client `lib/slskd_client.py` (the `slskd-api` PyPI dep is gone; legacy endpoints keep its dict/list return shapes, events return msgspec Structs with the double-decoded `data` payload). Phase 1 of the events refactor runs at the top of every poll cycle (`lib/slskd_events.py::ingest_download_file_events`): it pages `/api/v0/events` newest-first from a persisted single-row cursor (`slskd_event_cursor`), stamps each `DownloadFileComplete.localFilename` onto the matching `(username, remote filename)` file in `active_download_state`, and logs a resolver-vs-event comparison (`EVENT-PATH COMPARE`) in `process_completed_album`. **The resolver (`resolve_slskd_local_path`) still wins in phase 1** — flip preference to `local_path` (phase 2) only after a week of prod logs shows `match=False` never fires, then delete the resolver (phase 3).
