...
- **Triage** — `pipeline-cli triage show/list/quarantine` + `/api/triage/*`; composes unfindable + field-resolution + search forensics; surfaces unreferenced `failed_imports/`/`wrong_matches/` folders under the slskd download dir and processing tree (`<processing_dir>/albums/`; beets-staging is a known, unscanned gap). `quarantine` relays `GET /api/triage/quarantine`, since the processing tree's 0700 permission blocks direct CLI reads (#1122 F1). Read-only. `docs/search-plan-iter2-deploy.md`, `docs/pipeline-db-schema.md`.
