...
- **Beets** — deployment owns the compatible package, immutable effective config, SQLite catalog, files, state, secrets, current identity/path/tags, and plain operator `beet`; Cratedigger owns exact acquisition requests/history/proof. Its only mutation lanes are the serial JSON importer harness, the explicit exact-album delete child, and the import-time merge retag (`beet modify -a -M -W -y`, one exact-match-selected album, identity only, never moves files, reached by both the automation and force import lanes; #1087 replaced the original `beet mbsync -M` primitive, which cannot follow a release-only merge; #1093 replaced the query's anchored regex with an exact SQL-equality match, the same mechanism the post-retag guard already reads with); all other access is observational. Never raw-import or use `remove -d`; always match `candidate_id`, never `candidate_index`. `docs/beets-primer.md`.
