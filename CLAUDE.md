...
Schema lives in `migrations/NNN_name.sql`; the migrate oneshot runs them on every switch (`restartIfChanged = true` plus `stopIfChanged = false` — #1161: the latter routes it to switch-to-configuration's restart list, because a stop+start pair can have its queued stop replaced by any concurrent `systemctl start` and then silently no-op on `RemainAfterExit`). `cratedigger-web` and other long-running workers `requires` the migrate unit and start after it — that edge blocks them on a *failed* migration, but a `Requires=` on a `RemainAfterExit` oneshot cannot force a re-run; `cratedigger` and `cratedigger-unfindable` are timer-driven (`restartIfChanged = false`) so they only `wants`+`after` it — a `requires` edge would let the migrate unit's every-deploy restart SIGTERM a mid-flight cycle — and instead gate on schema currency themselves at startup (`lib/migrator.py::assert_schema_current`). Add a numbered SQL file — no manual psql, **never** edit a shipped migration, **never** add DDL inside `PipelineDB` methods. Full workflow in `.claude/rules/deploy.md`.
