...
- **slskd client + event ingestion** (issue #146) — all slskd HTTP calls go through the in-repo typed client `lib/slskd_client.py` (the `slskd-api` PyPI dep is gone; legacy endpoints keep its dict/list return shapes, events return msgspec Structs with the double-decoded `data` payload). Phase 1 of the events refactor runs at the top of every poll cycle (`lib/slskd_events.py::ingest_download_file_events`): it pages `/api/v0/events` newest-first from a persisted single-row cursor (`slskd_event_cursor`), stamps each `DownloadFileComplete.localFilename` onto the matching `(username, remote filename)` file in `active_download_state`, and `process_completed_album` moves files from the event-stream path. **Phase 2 is active: `local_path` wins when stamped and on disk; the resolver (`resolve_slskd_local_path`) is the logged fallback** (`EVENT-PATH: chosen=resolver` at WARNING — pre-bootstrap completions and missed events). Phase 3 (delete the resolver + ticks regex + size-match, wire `cancel_and_delete` to `DownloadDirectoryComplete`, then split download.py) is a **rate judgment, not a zero-grep**: known-benign `chosen=resolver` sources are pre-bootstrap completions, the residual completion-vs-event-write race within one snapshot fetch, and `cursor_gap` truncation on very heavy days — go ahead when every fallback line traces to that residue. Phase-1 evidence for the flip: 174 match=True, 0 match=False (2026-07-02).
