...
- **Triage** — `pipeline-cli triage show/list/quarantine` + `/api/triage/*`; composes unfindable + field-resolution + search forensics and surfaces unreferenced `failed_imports/`/`wrong_matches/` folders under the slskd download dir and `<processing_dir>/albums/` (beets-staging's pair: known unscanned gap). `quarantine` relays its API route (the `0700` processing tree blocks direct CLI reads, #1122). Read-only. `docs/search-plan-iter2-deploy.md`, `docs/pipeline-db-schema.md`.
