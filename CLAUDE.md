...
11. **Broken worlds surface and restart. Nothing is ever parked.** When the world turns out to be wrong — a failed move, a dead subprocess, an ambiguous acknowledgement, a DB blip, a half-finished cleanup — the response is always the same three steps: **record it as audit evidence** (so it renders in Recents and the operator can go fix the world), **return the request to a runnable state**, and **let the next cycle re-derive the truth**. Never leave a request in a state whose only exit is a human command. Such a request has silently stopped being acquired and nothing alerts on it — the operator finds out months later, or never. This follows from invariant 6: the request is the source of truth and everything else is derived, so restarting is always safe *because* the next cycle re-reads the world rather than trusting a snapshot; even "did the irreversible step already happen?" answers itself on re-derivation. **The smell to catch in review:** a new status, flag, queue state, or `*_required` marker whose only exit is an operator action; a failure branch that returns without leaving the request searchable; or a fallback that "preserves state for investigation". Ambiguity is not a reason to stop — it is a reason to record and retry. Retry cadence is handled by retained attempt counters and growing backoff, never by parking. Authority: "we surfsce broken worlds to the user and tje jsut restsrt, the pipeline keeps moving. never parked." — https://github.com/abl030/cratedigger/issues/898#issuecomment-5124769384
...
- **Triage** — `pipeline-cli triage show/list/quarantine` + `/api/triage/*`; composes unfindable + field-resolution + search forensics and surfaces unreferenced immediate album folders from the protected `failed_imports/` and `wrong_matches/` roots. Read-only. `docs/search-plan-iter2-deploy.md`, `docs/pipeline-db-schema.md`.
...
The importer queue is the beets-mutating ownership boundary: web/CLI/poller enqueue; `cratedigger-importer` drains serially under an advisory singleton lock. A request-backed automation job additionally owns its request for the complete `processing` lifetime. A world failure anywhere in that lifetime — including a launched-but-unacknowledged beets child — records audit evidence and returns the request to `wanted` for the next cycle to re-derive; it is never parked for an operator (invariant 11). No new direct beets-mutating entry points outside the importer worker.
