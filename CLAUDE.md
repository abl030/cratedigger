...
The importer queue is the beets-mutating ownership boundary: web/CLI/poller enqueue; `cratedigger-importer` drains serially under an advisory singleton lock. A request-backed automation job additionally owns its request for the complete `processing` lifetime. Startup requeues only an exact pre-launch owner whose persisted execution is positively dead; launched or ambiguous owners become/stay `recovery_required` for revision-bound operator retry/close. No new direct beets-mutating entry points outside the importer worker.
