...
- **slskd client + event ingestion** — all slskd HTTP via the in-repo typed client `lib/slskd_client.py`. Event ingestion (`lib/slskd_events.py`) stamps completed-file locations from the slskd events feed; **the event is the ONLY source of completed-file locations** (unstamped file at materialize = hard failure, then self-heal to `wanted`). Deletions are event-driven paths + empty-dir pruning — never an inferred-folder rmtree. **Good-citizen ownership (#571, reversing #550's own-the-instance doctrine): cratedigger only destroys slskd state/files it can positively prove it created, via write-ahead ledgers — a shared instance is safe.** `converge_slskd_orphans` cancels ledger-owned STRAY transfers only — present in `slskd_transfer_ledger` but no longer backed by a `downloading` row — and NEVER a foreign/unledgered transfer, whatever its state or age (#571 PR 3; flipped from the old "cancel anything unowned" doctrine, which risked cancelling a human's transfer on a shared slskd instance). `reap_disk_orphans` (Phase 0b, `lib/slskd_transfers.py`) deletes files older than 7 days that it can positively prove it created — good-citizen doctrine (#571 PR 4): a ledgered event-stamped `local_path`, or under a canonical folder derived from a ledgered `attempt_fingerprint` (past OR present attempts). A file with no positive ownership signal is NEVER deleted however old (pre-#571 debris from before migration 045 shipped included — an operator one-shot clears that at deploy, not the reaper's job going forward); the `failed_imports/` quarantine tree (cratedigger's own tree by construction, but protected — never reap-eligible) and a currently-`downloading` row's active canonical folder/stamped paths stay protected regardless of ledger ownership or age. Never park files there. Searches cratedigger submits are write-ahead ledgered (migration 044) and reaped by `converge_slskd_searches` (Phase 0c, `lib/slskd_searches.py`); unledgered searches are never touched (#576). Every slskd transfer cratedigger enqueues is likewise write-ahead ledgered (migration 045, `slskd_enqueue_with_outcome` in `lib/slskd_transfers.py`, before the POST). Its `transfer_id` is captured right after POST reconciliation (T1.5), or COALESCEd from a successful completion event (T2). The end-of-cycle completed-transfer purge first harvests terminal evidence, then converges every owned terminal outcome into `completed_at`: success only through the authoritative event/local-path write; non-success `Completed,*` states through an exact-ID stamp. A failure that raced T1.5 may atomically claim one still-open, exact peer/file T1 row only when that row predates slskd's request timestamp, preventing an old terminal record from binding a newer retry. Only IDs confirmed by those durable writes are removed, individually with `remove=true`; unconfirmed/foreign transfers and nonterminal rows are untouched. Phase 0d (`lib/slskd_transfer_ledger.py`) prunes rows once old AND their request is inactive (retention strictly exceeds the reaper's own age threshold — a pruned row's file becomes unowned and is never reaped after that).
  Terminal evidence has three states: event-stamped `local_path` authorizes success removal, pathless `completed_at` authorizes only failure removal, and an open row authorizes neither. An exact-ID success event upgrades a pathless failure stamp. Non-success fallback claims require slskd's own `requestedAt` (never local `enqueued_at`) and the five-minute causal window. Migration 049 makes every non-NULL transfer ID globally unique while retaining duplicate historical rows with their extra IDs nulled.
