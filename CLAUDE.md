...
7. **Don't duplicate convergence — reuse the cleanup paths that already exist.** When an operator action could leave behind orphans (in-flight slskd transfers, stale staging, dangling rows), prefer letting existing convergence pick them up over adding bespoke teardown to the action. Where convergence does not yet exist, file an issue and ship the closest direct cleanup in the action itself. Replace deliberately leaves in-flight slskd transfers running rather than building a partial cancellation path that would duplicate that work — the Phase 0 orphan-transfer convergence (`lib/slskd_transfers.py::converge_slskd_orphans`, #278) reaps them at the top of the next cycle by cancelling any live transfer no `downloading` row owns.
...
- **Beets** (cratedigger-owned: pinned package `nix/beets.nix`, module-rendered config at `${stateDir}/beets` (BEETSDIR), `cratedigger-beet` as the manual-ops binary — colocated with cratedigger, currently doc2) — library source of truth. All automated imports go through the JSON harness (`harness/beets_harness.py` via `run_beets_harness.sh`, which execs the pinned interpreter from `CRATEDIGGER_BEETS_PYTHON`), never raw `beet import`. The `musicbrainz` plugin MUST be in the plugins list or beets returns 0 candidates (the moduleVm asserts it). Always match by `candidate_id` (MB release UUID), never `candidate_index`. See `docs/beets-primer.md`.
...
- **MusicBrainz mirror** (`http://192.168.1.35:5200`) — local MB mirror. Threaded everywhere from ONE value (`services.cratedigger.musicbrainz.apiBase`); public-MB is the supported-but-slow fallback. See `docs/musicbrainz-mirror.md` and `docs/mirrors.md` (system requirements + runbook).
...
- **doc2** (`192.168.1.35`): runs cratedigger (systemd oneshot, 5-min timer) and its module-owned beets, plus MusicBrainz mirror (`:5200`) and slskd (`:5030`). Beets is colocated with cratedigger so the harness can invoke the pinned beets locally.
